...
To mitigate the well-known risks of compiling Solidity to the WebAssembly programming language (WASM), we're not working directly in Solidity. Instead, we're building the reference SDK in Rust, because it produces very succinct WASM code and keeps runtime costs down.
